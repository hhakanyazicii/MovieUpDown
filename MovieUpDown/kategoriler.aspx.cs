...
namespace MovieUpDown
{
    public partial class kategoriler : System.Web.UI.Page
    {
        ...
        {
            var kategoriID = Request.QueryString["id"];
            ...
            SqlDataAdapter daKategori = new SqlDataAdapter("SELECT film.id,film.adi,film.cikis_tarihi,film.konu,film.img_path,kategori.kategori_adi FROM film INNER JOIN kategori ON film.kategori_id = kategori.id WHERE kategori.id='" + kategoriID + "'", baglanti); // create a DataSet object to hold you table(s)... this can contain more than 1 table
            daKategori.Fill(dsKategori);

            for (int i = 0; i < dsKategori.Tables[0].Rows.Count; i++)
            {
                ...
                postImg.Attributes["src"] = dsKategori.Tables[0].Rows[i]["img_path"].ToString();
                var postH1 = new HtmlGenericControl("h1")
                {
                    InnerText = dsKategori.Tables[0].Rows[i]["adi"].ToString()
                };
                var h1Anc = new HtmlGenericControl("a");
                h1Anc.Attributes["href"] = "/filmdetay.aspx?id=" + dsKategori.Tables[0].Rows[i]["id"].ToString();
                h1Anc.Controls.Add(postH1);

                var postH4 = new HtmlGenericControl("h4")
                {
                    InnerText = dsKategori.Tables[0].Rows[i]["cikis_tarihi"].ToString()
                };
                var postP2 = new HtmlGenericControl("p") { InnerText = dsKategori.Tables[0].Rows[i]["kategori_adi"].ToString() };
                ...
                var postP = new HtmlGenericControl("p") { InnerText = dsKategori.Tables[0].Rows[i]["konu"].ToString() };
                ...
                postLastDiv.Attributes["style"] = "background-image: url('" + dsKategori.Tables[0].Rows[i]["img_path"].ToString() + "');";
                postDiv.Controls.Add(postDiv2);
                postDiv.Controls.Add(postLastDiv);
                kategoriPanel.Controls.Add(postDiv);
            }
        }
    }
}